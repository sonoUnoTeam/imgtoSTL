...
pv = np.zeros((h, w, c), dtype=np.uint8)
x = np.zeros((h, w))
y = np.zeros((h, w))
zr = np.zeros((h, w))
zb = np.zeros((h, w))
...
for i in range(h):
    for j in range(w):
        ...
        pG = g / 255.0 * 100  # PORCENTAJE DE VERDE
        #Amarillo Claro
        if 100 >= pR >= 80 and 100 >= pG >= 60 and 70 >= pB >= 20:
            if pixel[0] == 0 and pixel[1] == 0 and pixel[2] == 0:
                x[i, j] = j
                y[i, j] = i
                zy[i, j] = 5
        ...
        # Amarillo
            ...
            py[i, j] = pixel
            # Verde Claro
        elif 80 >= pR >= 40 and 100 >= pG >= 70 and 85 >= pB >= 0:
            if pixel[0] == 0 and pixel[1] == 0 and pixel[2] == 0:
                x[i, j] = j
                y[i, j] = i
                zg[i, j] = 5
            ...
            pb[i, j] = pixel
            # Violeta 
        elif 25 >= pR >= 10 and 12 >= pG >= 5 and 45 >= pB >= 30:
            if pixel[0] == 0 and pixel[1] == 0 and pixel[2] == 0:
                x[i, j] = j
                y[i, j] = i
                zp[i, j] = 5
...
mesht = pv.merge([meshw, meshr, meshb, meshy, meshp, meshc, meshg, meshbl, mesho, meshbr,meshv, base]) # Add mesh
